...
const zhTW: SiteContent = {
  ...
  caseStudies: [
    ...
    },
    {
      ...
      company: "Munto（문토）",
      title: "解決無 TTL 設定導致的 Redis 快取記憶體暴增問題",
      subtitle: "將 Lounge 動態快取記憶體使用率從 90% 改善至 58%",
      summary: "Munto 的 Lounge 快取因未設定 TTL，導致特定熱門使用者的 key 無限增長，使 Redis 記憶體使用率飆升至 90%，透過數據分析找出並解決了這個問題。藉由抽樣 element 數量分布，確認前 1% 使用者的 key 佔據了大部分記憶體，並以 p90 為基準設定 500 筆上限（cap），採漸進式上線的方式因應。最終將記憶體使用率降至 58%，且未產生任何副作用。",
      tags: ["Redis", "CloudWatch", "SNS", "AWS Chatbot", "Slack", "ZSET"],
      steps: [
        {
          label: "問題",
          body: "Munto 的 Lounge 快取用於儲存使用者所追蹤對象的動態，並在熱門貼文等精選區塊中呈現，而這個快取的記憶體使用率一路飆升至 90%。過去團隊僅靠手動檢視儀表板來發現並回報問題，但在建置了 CloudWatch Alarm → SNS → AWS Chatbot 串接 Slack 的告警體系後，才得以及時察覺這個異常。所幸在發現當下，尚未對服務造成實際故障或回應延遲等影響。",
        },
        {
          label: "原因",
          body: "追查後發現，整體記憶體有 60% 來自單一一個快取 key。這個 key 對應的是追蹤者眾多的熱門使用者動態，由於未設定 TTL，新動態不斷被 append 進去，導致該 key 無止盡地膨脹。至於這段舊有程式碼當初為何沒有設定 TTL，已無從查證。",
        },
        {
          label: "取捨",
          body: "單純補上 TTL 的方案，因無法確認當初未設定 TTL 的原因，擔心引發未知的副作用而暫緩採用。變更 key 結構或導入快取容量限制的方案，考量到開發資源負擔過大而排除；直接擴充快取實例記憶體的方案雖也曾評估，但這只是放任舊有結構持續累積問題，因此未採用。最終選擇先抽樣 element 數量分布（2,015 筆樣本，p50 為 48 筆、p90 為 504 筆、p99 為 2,195 筆、最大值為 22,415 筆），確認前 1% 使用者的 ZSET 佔據了大部分記憶體後，判斷縮小範圍、漸進式上線是投入資源與效果比最划算的方向。",
        },
        {
          label: "決策",
          body: "決定以 p90 為基準，將快取 key 的 element 數量上限設為 500 筆。利用 ZSET 的特性，實作成當新動態進入時，會自動淘汰（trim）最舊的資料；既有的 key 維持原狀不動，僅針對之後新產生的 key 逐步套用此限制上線。",
        },
        {
          label: "結果",
          body: "套用變更後，Redis 記憶體使用率從 90% 降至 58%。上線後，即便舊動態被裁切，也尚未觀察到任何使用者抱怨等副作用。",
        },
        {
          label: "心得",
          body: "體會到導入快取時，應將設定 TTL 的依據明確記錄下來，日後才能更容易追查原因並因應問題。也建立了一項原則：即使日後再遇到沒有 TTL 的舊設定，也不該貿然直接補上，而是要先充分評估可能的副作用再著手處理。此外也確認到，單純擴充快取記憶體這種便宜行事的做法，只會讓舊有結構持續累積，並非根本的解決方案。",
        },
      ],
    },
  ],
