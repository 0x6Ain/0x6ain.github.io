...
const ja: SiteContent = {
  ...
  caseStudies: [
    ...
    },
    {
      ...
      company: "Munto(ムント)",
      title: "TTL未設定によるRedisキャッシュのメモリ肥大化を解決",
      subtitle: "ラウンジフィードキャッシュのメモリ使用率を90%→58%に改善",
      summary: "Muntoのラウンジキャッシュで、TTLが設定されていなかったために一部の人気ユーザーのキーが無限に肥大化し、Redisのメモリ使用率が90%まで上昇した問題をデータドリブンに分析・解決した。element数の分布をサンプリングした結果、上位1%のユーザーキーがメモリの大半を占めていることを確認し、p90を基準に500件のcapを設定して段階的にロールアウトした。結果としてメモリ使用率を58%まで下げ、副作用なく対応を完了した。",
      tags: ["Redis", "CloudWatch", "SNS", "AWS Chatbot", "Slack", "ZSET"],
      steps: [
        {
          label: "課題",
          body: "Muntoのラウンジキャッシュは、ユーザーがフォローした対象のフィードを保存し、人気投稿などのスポットライト領域に表示するためのRedisキャッシュだったが、このキャッシュのメモリ使用率が90%まで急上昇した。以前はダッシュボードを手動で確認して問題を検知する程度だったが、CloudWatch Alarm→SNS→AWS Chatbotを経由してSlackに通知を連携するモニタリング体制を構築していたことで、この問題を早期に把握できた。幸い、この時点ではサービス障害や応答遅延といった実質的な影響はまだ出ていなかった。",
        },
        {
          label: "原因",
          body: "原因を追跡した結果、全体メモリの60%がたった1つのキャッシュキーに集中していることが判明した。このキーはフォロワー数の多い人気ユーザーのフィードで、TTLが設定されていなかったため新しいフィードが追加され続け、無限に肥大化する構造になっていた。レガシーコードにそもそもTTLが設定されていなかった理由は、記録からは確認できなかった。",
        },
        {
          label: "トレードオフ",
          body: "単純にTTLを追加する方法は、そもそもなぜTTLなしで設計されたのか根拠が不明確で、副作用のリスクがあるため保留した。キー構造の変更やキャッシュサイズ制限の導入は開発リソースの負担が大きいと判断し除外し、キャッシュインスタンス自体のメモリを増強する案も検討したが、レガシーが蓄積し続けている構造を放置することになるため採用しなかった。代わりにelement数の分布をサンプリング(2,015件、p50が48件・p90が504件・p99が2,195件・最大22,415件)し、上位1%のユーザーのZSETがメモリの大半を占めていることを確認した上で、範囲を絞って段階的にロールアウトする方法が最もリソース対効果の高いアプローチだと判断した。",
        },
        {
          label: "決定",
          body: "p90を基準にキャッシュキーのelement数を500件に制限(cap)することを決定した。ZSETの特性を活かし、新しいフィードが入る際に最も古い値から入れ替わる(trim)ように実装し、既存のキーはそのままにして、以後新たに発生するキーから段階的にロールアウトした。",
        },
        {
          label: "結果",
          body: "変更を適用した後、Redisのメモリ使用率は90%から58%まで下がった。ロールアウト後、古いフィードが削られることによるユーザーからの不満などの副作用も、現時点では確認されていない。",
        },
        {
          label: "学び",
          body: "キャッシュを導入する際は、TTLを設定した根拠を明文化しておくことで、後から原因を突き止め対応しやすくなることを学んだ。TTLのないレガシー設定に再び遭遇した場合も、むやみにTTLを追加するのではなく、副作用を十分に検討した上でアプローチすべきという原則を立てた。また、キャッシュのメモリ自体を増やす手軽な方法は、レガシー構造が蓄積し続ける結果を招きかねず、根本的な解決策にはならないことも確認できた。",
        },
      ],
    },
  ],
